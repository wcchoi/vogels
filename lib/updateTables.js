...
internals.updateTable = function (model, options, callback) {
  ...
  model.describeTable(function (err, data) {
    if(!_.isNull(data)) {
      ...
      return model.updateTable(options, existingIndexes, existingThroughput, function (error, data) {
        ...
        if(data) {
          console.log('waiting for table ' + tableName + '\'s indexes to become ACTIVE');
...
module.exports = function (models, config, callback) {
    var tasks = _.map(_.keys(models), function(key) {
        return function(callbackTask) {
            return internals.updateTable(models[key], config[key], callbackTask);
        }
    });
